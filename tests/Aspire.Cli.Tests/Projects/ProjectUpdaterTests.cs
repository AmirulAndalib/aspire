...
public class ProjectUpdaterTests(ITestOutputHelper outputHelper)
{
    ...
    public async Task UpdateProjectFileAsync_DoesAttemptToUpdateIfNoUpdatesRequired()
    {
        ...
        var updateResult = await projectUpdater.UpdateProjectAsync(new UpdatePackagesContext { AppHostFile = appHostProjectFile, Channel = selectedChannel }).DefaultTimeout();
    ...
    public async Task UpdateProjectFileAsync_CentralPackageManagement_PropertyResolutionFailsWithInvalidSemanticVersion()
    {
        ...
        {
            await projectUpdater.UpdateProjectAsync(new UpdatePackagesContext { AppHostFile = appHostProjectFile, Channel = selectedChannel }).DefaultTimeout();
        });
    ...
    public async Task FallbackMode_InvalidXml_StillFails()
    {
        ...
        await Assert.ThrowsAsync<ProjectUpdaterException>(() =>
            projectUpdater.UpdateProjectAsync(new UpdatePackagesContext { AppHostFile = appHostProjectFile, Channel = selectedChannel }).DefaultTimeout());
    }
    ...
    public async Task UpdateProjectFileAsync_SingleFileAppHost_UpdatesSdkDirectiveWithVersion()
    {
        ...
        var projectUpdater = new ProjectUpdater(logger, runner, interactionService, cache, executionContext, fallbackParser);
        var updateResult = await projectUpdater.UpdateProjectAsync(new UpdatePackagesContext { AppHostFile = appHostFile, Channel = selectedChannel }).DefaultTimeout();
