...
- [x] Write `DotnetToolSmokeTests.cs` — install via `dotnet tool install`, then `aspire new` + `aspire run`
- [ ] Verify it works in CI with the uploaded nupkg artifact

## Status

**CI plumbing + test class committed.** The test installs via `dotnet tool install`
from a mounted nupkg directory in Docker, then runs `aspire new` + `aspire run`.
