...
internal abstract class PipelineCommandBase : BaseCommand
{
    ...
    private async Task<string?> HandleSingleInputAsync(PublishingPromptInput input, string promptText, CancellationToken cancellationToken)
    {
        ...
        return inputType switch
        {
            InputType.Text => await InteractionService.PromptForStringAsync(
                promptText,
                binding: PromptBinding.CreateDefault(input.Value?.EscapeMarkup()),
            ...
            _ => await InteractionService.PromptForStringAsync(promptText, binding: PromptBinding.CreateDefault(input.Value?.EscapeMarkup()), required: input.Required, cancellationToken: cancellationToken)
        };
    }
    ...
    private async Task<string?> HandleSelectInputAsync(PublishingPromptInput input, string promptText, CancellationToken cancellationToken)
    {
        ...
        {
            return await InteractionService.PromptForStringAsync(promptText, binding: PromptBinding.CreateDefault(input.Value?.EscapeMarkup()), required: input.Required, cancellationToken: cancellationToken);
        }
